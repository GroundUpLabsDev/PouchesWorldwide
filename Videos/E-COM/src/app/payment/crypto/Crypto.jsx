...
🚀 Pouches`,
  ...
  const handleProceed = async () => {
    if (!transactionId.trim()) {
      alert("Please enter a transaction ID before proceeding.");
      return;
    }
    ...
    if (selectedFile) {
      ...
      try {
        const uploadResponse = await fetch("https://pouchesworldwide.com/strapi/api/upload", {
          method: "POST",
          body: formData,
        });
        ...
        if (uploadResponse.ok) {
          ...
          uploadedFileId = uploadData[0].id; // Store the uploaded file ID
        } else {
    ...
    const orderData = {
      data: {
        txid: transactionId,
        btc: uploadedFileId, // Include the uploaded file ID in the `btc` field
        user: userId, 
        timeline: [
          {
