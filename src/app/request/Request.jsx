...
export default function Request() {
  ...
  const updateTotalPrice = () => {
    return requestedProducts.reduce((total, product) => total + product.quantity * product.price, 0);
  };
  ...
  const handleRequestSubmit = async () => {
    try {
      ...
      // Send each product as a separate order
      for (const product of requestedProducts) {
        const payload = {
          data: {
            ...
            address: addressJson, // Send address as a JSON object
            Can: product.quantity, // Quantity of the product
        ...
        // Send the order to the new API endpoint
        const response = await fetch("https://pouchesworldwide.com/strapi/api/corders", {
          method: "POST",
          headers: {
        ...
        if (!response.ok) {
          throw new Error(`Failed to create order for product ${product.id}`);
        }
      }

      router.push("/cart"); // Redirect to cart page after all orders are created
    } catch (error) {
        ...
        <div className="w-full max-w-4xl">
          ...
          {requestedProducts.map((product, index) => (
            <OrderRequestDetails
              ...
              updateTotalPrice={updateTotalPrice} // ✅ Pass the callback
            />
          ))}
        </div>
      ...
      <div className="flex items-center p-6 ml-[500px]">
        <div className="flex items-center space-x-[102px]">
          <div className="flex items-center space-x-4">
            ...
            <span className="text-primary font-semibold text-[28px]">
            ${requestedProducts.reduce((total, product) => total + product.quantity * product.price, 0).toFixed(2)}
